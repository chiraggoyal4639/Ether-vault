...
contract Interact is Script {
    ...
    function doPauseTest() internal {
        ...
        // Attempt deposit as EOA (should fail)
        vm.startPrank(user);
        try vault.deposit{value: 1 ether}() {
            console.log("Deposit unexpectedly succeeded");
        } catch {console.log("Deposit correctly failed during pause");}
        vm.stopPrank();
        }
    function doUnpauseTest() internal {
        ...
        } catch {console.log("Deposit unexpectedly failed");}
        vm.stopPrank();
        }
    function doWithdrawTest(address _user) internal {
        uint256 lastDepositTime = vault.lastDepositAt(msg.sender);
        uint256 cooldown = vault.depositCooldown();
        vm.warp(lastDepositTime + cooldown + 1);
        vm.startPrank(_user);
